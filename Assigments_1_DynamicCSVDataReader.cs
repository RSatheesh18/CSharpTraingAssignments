...
public class CSVDataProvider
{
    ...
    public IEnumerable<CsvLineInfo> GetPProfessionalDataRecords()
    {
        //Parse the File 
        //Etract Lini By Line
        //Construct CsvLineInfo Object
        CsvLineInfo _line1 = new CsvLineInfo() { header = "Company,Industry,Role,Area", lineContent = "Philips,IT,Developer,BLR" };
        CsvLineInfo _line2 = new CsvLineInfo() { header = "Company,Designation,Role,Area", lineContent = "Tata,Indutry,QualityEngineer,Koramangala"  };
        CsvLineInfo _line3 = new CsvLineInfo() { header = "Company,Designation,Role,Area", lineContent = "Volvo,Mechanics,tester,BTM" };
        CsvLineInfo _line4 = new CsvLineInfo() { header = "Company,Designation,Role,Area", lineContent = "Philips,It,tester,BLR" };
        List<CsvLineInfo> _objectList = new List<CsvLineInfo>();
        _objectList.AddRange(new CsvLineInfo[] { _line1, _line2, _line3, _line4 });
        return _objectList;
    }
...
public class Program
{
    public static void Main()
    {
        ...
        CSVDataProvider _provider = new CSVDataProvider() { FilePath = "patients.csv" };
       // _provider.FilePath = "patients.csv";
        ...
        IEnumerable<CsvLineInfo> patientProfessinalData = _provider.GetPProfessionalDataRecords();

         result = Query(patientProfessinalData, (dynamic patient) => { return patient.Company == "Philips"; });
        foreach (dynamic patient in result)
        {
            Console.WriteLine(patient.ToString());
        }
